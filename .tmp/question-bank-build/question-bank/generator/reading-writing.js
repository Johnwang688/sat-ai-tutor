...
exports.readingWritingQuestionBank = exports.readingWritingQuestionGenerators = exports.readingWritingConceptSlugs = void 0;
exports.generateReadingWritingQuestionsForConcept = generateReadingWritingQuestionsForConcept;
exports.generateReadingWritingQuestionBank = generateReadingWritingQuestionBank;
const core_concepts_1 = require("../../taxonomy/core-concepts");
const sectionSlug = "reading-writing";
const sectionName = "Reading and Writing";
const sourceTag = "generated_from_research";
...
function resolveConcept(slug) {
    const concept = (0, core_concepts_1.getCanonicalConcept)(slug);
    if (!concept) {
...
function toChoiceTuple(texts) {
    return texts.map((text, index) => ({
        label: choiceLabels[index] ?? "A",
        text,
    }));
}
function rotateChoices(texts, rotation) {
...
function buildFiveFactThemeQuestions(args) {
    return args.facts.map((fact, index) => {
        ...
        const answerChoices = rotateChoices([fact, distractors[0] ?? args.facts[1], distractors[1] ?? args.facts[2], distractors[2] ?? args.facts[3]], index);
...
function buildItemThemeQuestions(args) {
    return args.items.map((item, index) => makeQuestionRecord({
        concept: args.concept,
...
const centralIdeaThemes = [
    {
        id: "tool-library",
        passage: "A neighborhood tool library opened in an unused bus depot. Members can borrow ladders, drills, and garden tools instead of buying items they will use only once or twice. The program also offers short weekend workshops for simple repairs.",
        facts: [
    ...
    {
        id: "rooftop-garden",
        passage: "An apartment complex turned its flat roof into a garden with raised beds, rain barrels, and a small herb corner. Neighbors water the plants in rotating shifts, and the harvest is shared among the building's residents. The garden has also become a place where children learn how compost supports new growth.",
        facts: [
...
function generateCentralIdeasAndDetails() {
    const concept = resolveConcept("central-ideas-and-details");
    return centralIdeaThemes.flatMap((theme, themeIndex) => buildFiveFactThemeQuestions({
        concept,
        themeId: theme.id,
...
const evidenceTextualThemes = [
    {
        id: "transit-passes",
        passage: "City planners tested monthly bus passes that cost less than the usual fare bundle. Riders with the passes took more trips in the evening, and they reported fewer missed connections because they were less worried about each transfer. The report concluded that the new pass made transit feel more flexible.",
    ...
    {
        id: "tree-planting",
        passage: "Volunteers planting shade trees on a hot schoolyard tracked the difference in temperature before and after the project. The surface near the trees stayed cooler at midday, and teachers said students now used the courtyard more often during lunch. The group shared the results with the district to support future planting plans.",
...
function generateCommandOfEvidenceTextual() {
    const concept = resolveConcept("command-of-evidence-textual");
    return evidenceTextualThemes.flatMap((theme, themeIndex) => buildFiveFactThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
const quantitativeEvidenceThemes = [
    {
        id: "library-usage",
        table: "A survey of 120 students found that 72 borrowed a book more than once a month, 36 borrowed books only a few times a semester, and 12 rarely borrowed books.",
    ...
    {
        id: "commute-times",
        table: "A transportation study tracked average commute times for three neighborhoods. Neighborhood A averaged 18 minutes, Neighborhood B averaged 26 minutes, and Neighborhood C averaged 31 minutes after a bus route change.",
    ...
    {
        id: "garden-harvest",
        table: "A school garden logged produce over four weeks: week 1, 14 pounds; week 2, 17 pounds; week 3, 21 pounds; week 4, 24 pounds.",
    ...
    },
    {
        ...
        table: "A weekend workshop series drew 40 attendees on day one, 55 on day two, and 58 on day three. Staff said the higher turnout happened after the event was advertised more widely online.",
...
function generateCommandOfEvidenceQuantitative() {
    const concept = resolveConcept("command-of-evidence-quantitative");
    return quantitativeEvidenceThemes.flatMap((theme, themeIndex) => buildFiveFactThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
const inferenceThemes = [
    {
        id: "late-shift-bakery",
        passage: "The bakery opens before sunrise, and the first tray of bread is usually gone by 7:30 a.m. Customers line up outside even when the weather is cold because the owner bakes only small batches. By noon, the shelves are empty and the oven is already cooling for the next day.",
        facts: [
    ...
    {
        id: "field-notes",
        passage: "A student club spent the semester taking notes about birds in the park and comparing sightings from one week to the next. Their chart showed that certain birds appeared more often after the club planted native shrubs. The advisor said the club had begun to notice how habitat choices affect wildlife.",
        facts: [
    ...
    {
        id: "repair-cafe",
        passage: "At the repair cafe, volunteers fixed lamps, hemmed jackets, and replaced missing buttons while owners waited at nearby tables. People left carrying items they had thought were useless only an hour earlier. The organizers said the event worked because it paired practical help with a relaxed social setting.",
        facts: [
...
function generateInferences() {
    ...
    return inferenceThemes.flatMap((theme, themeIndex) => buildFiveFactThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
function generateWordsInContext() {
    const concept = resolveConcept("words-in-context");
    return wordsInContextThemes.flatMap((theme, themeIndex) => buildItemThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
const textStructureThemes = [
    {
        id: "opening-anecdote",
        passage: "The article opens with a description of a child tracing raindrops down a window. It then shifts to a discussion of how water cycles through a city. The first image is used to move the reader from a familiar scene to a larger scientific point.",
        facts: [
    ...
    },
    {
        ...
        passage: "The first paragraph describes a park that looked dull in winter. The second paragraph explains how the same park becomes active in spring, and the final sentence argues that the change reveals the value of patient planning. The arrangement lets the writer contrast appearance and long-term effect.",
        facts: [
    ...
    },
    {
        ...
        passage: "The author defines a maker space as a room where people can test ideas, then gives examples such as sewing machines, 3-D printers, and shared worktables. The explanation helps readers understand that the term refers to both tools and the habits of collaboration that form around them.",
        facts: [
    ...
    },
    {
        ...
        passage: "After describing several short experiments, the writer concludes that small adjustments can lead to larger improvements. The final sentence does not introduce new evidence; instead, it ties the examples together and signals the article's main lesson.",
        facts: [
...
function generateTextStructureAndPurpose() {
    const concept = resolveConcept("text-structure-and-purpose");
    return textStructureThemes.flatMap((theme, themeIndex) => buildFiveFactThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
function generateCrossTextConnections() {
    const concept = resolveConcept("cross-text-connections");
    return crossTextThemes.flatMap((theme, themeIndex) => buildFiveFactThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
function generateTransitions() {
    ...
    return transitionThemes.flatMap((theme, themeIndex) => buildItemThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
function generateRhetoricalSynthesis() {
    const concept = resolveConcept("rhetorical-synthesis");
    return synthesisThemes.flatMap((theme, themeIndex) => buildItemThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
function generateBoundaries() {
    ...
    return boundaryThemes.flatMap((theme, themeIndex) => buildItemThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
function generateFormStructureAndSenseGrammar() {
    const concept = resolveConcept("form-structure-and-sense-grammar");
    return grammarThemes.flatMap((theme, themeIndex) => buildItemThemeQuestions({
        concept,
        themeId: theme.id,
        baseIndex: themeIndex * 5,
...
exports.readingWritingConceptSlugs = [
...
exports.readingWritingQuestionGenerators = {
...
function generateReadingWritingQuestionsForConcept(conceptSlug) {
    return exports.readingWritingQuestionGenerators[conceptSlug]();
}
function generateReadingWritingQuestionBank() {
    return exports.readingWritingConceptSlugs.flatMap((conceptSlug) => generateReadingWritingQuestionsForConcept(conceptSlug));
}
exports.readingWritingQuestionBank = generateReadingWritingQuestionBank();
